	...
	const handleSort = () => {
		...
		for (let i = 0; foundClass.length > i; i++) {
			if (searchedClass.class_type === foundClass[i].class_type) {
				...
				searchedClass.class_start_time === foundClass[i].class_start_time
			) {
				found.push(foundClass[i]);
			} else if (
				searchedClass.class_duration === foundClass[i].class_duration
			) {
				found.push(foundClass[i]);
			} else if (
				searchedClass.class_intensity_level ===
				foundClass[i].class_intensity_level
			) {
				found.push(foundClass[i]);
			} else if (
				searchedClass.class_location === foundClass[i].class_location
			) {
				found.push(foundClass[i]);
			}
			setState(found);
					...
					<ComponentContainer>
						<ModalContainer>
							<div>
								...
								<h4>
									Attendees: {s.total_clients} people (Max: {s.max_class_size}{' '}
									people)
								</h4>
							</div>
						</ModalContainer>
